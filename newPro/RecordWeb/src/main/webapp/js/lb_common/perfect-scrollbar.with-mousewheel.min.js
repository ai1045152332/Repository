(function($) {
    $.fn.perfectScrollbar = function(option) {
        ...
        var $this = $(this).addClass("ps-container"),
            ...
            $scrollbar_y = $("<div class='ps-scrollbar-y' id='scrolly'></div>").appendTo($this),
            container_width,
            container_height,
            content_width,
            content_height,
        ...
        var updateBarSizeAndPosition = function() {//changesize方法调用此处
            ...
            $scrollbar_y.css({//changesize赋值操作
            ...
            //赋值修改
            //alert($scrollbar_y.css("top")+"--"+$scrollbar_y.css("height"))
            var changetop=$("#perfectscrollval").attr("num");
            //alert(changetop+"---"+$scrollbar_y.css("top"))
            if(parseInt(scrollbar_y_top + $this.scrollTop()+scrollbar_y_height)>=parseInt(changetop)){
            	$scrollbar_y.css({//changesize赋值操作
	                top: parseInt(changetop)-scrollbar_y_height
	            })
            }else{
            	$scrollbar_y.css({//changesize赋值操作
	                top: scrollbar_y_top + $this.scrollTop()
            ...
            $("#perfectscrollval").attr("top",parseInt($scrollbar_y.css("top")))
            $("#perfectscrollval").attr("height",parseInt($scrollbar_y.css("height")))
        ...
        var bindMouseWheelHandler = function() {
            ...
            $this.mousewheel(function(e, delta, deltaX, deltaY) {
                ...
                var allheight=parseInt($("#scrolly").css("height"))+parseInt($("#scrolly").css("top","0"));
                //alert(allheight)
               // alert(allheight-thisheight)
               //alert(allheight+"--"+parseInt($this.children("div").eq(0).height()))
               if(allheight>=parseInt($this.children("div").eq(0).height())){
                   if($("#perfectscrollval").attr("which")=="controlscroll"){
                       $this.scrollTop($this.scrollTop() - 49);
                   }else{
                       $this.scrollTop(allheight-thisheight-0.1);
                   }
                ...
                updateBarSizeAndPosition();//滑轮滚动执行一次
        ...
        var bindMobileTouchHandler = function() {
            var applyTouchMove = function(difference_x, difference_y) {
                ...
                updateBarSizeAndPosition()//手机触屏执行一次
            };
            var start_coords = {},
                start_time = 0,
                speed = {},
        ...
        var initialize = function() {
            updateBarSizeAndPosition();//页面初始化执行一次，值为nan
