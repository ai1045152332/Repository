                                ...
                                <span class="tree_titlea" groupId="${group_id}"
                                    ...
                                    <div class="tree_content">
                                        ...
                                        <s:iterator value="%{hostInfoList}" id="inner">
                                            <div class="tree_contenta" hostId="${id}" hostIp="${token}" hostName="${name}"  status ="${status}">
                                                <s:if test="%{status=='Offline'}">
                                                    <span class="tree_content_nousebg" style="background: none;"></span>
                                                </s:if>
                                                <s:else>
                                                    <span class="tree_content_onlinebg"
    ...
    <div class="public_right floatleft">
        <div class="jkn_message_rightcenter">
            ...
            <div class="jkn_message_90 floatleft">
                ...
                <div class="jkn_message_radio2"  style="margin-top: 0; margin-left: 30px;width:auto">
                    <div class="headr">
                        <span id="ringbell" class="bgr"></span>响铃
                    </div>
        ...
        <div class="jkn_message_rightcenter hei_50 floatleft" style="margin-left: 5%;">
            ...
            <%
                int i = 0;
            %>
            <s:iterator value="#ringbellList" status='st'>
                <div id="${ringId}"
                     isloop="${ringLoop}"
                     looptype="${ringLooptype}"
                     date="${ringDate}"
                     week="${ringWeek}"

                     time="${ringTime}"
                     singletime="${ringSingletime}"
                     uid="${ringUid}"

                     class="<%=i%2==0? "jkn_message_listwhite floatleft" : "jkn_message_listgray floatleft"%>">
                    <fmt:parseDate value="${ringCreatetime}" var="parsedEmpDate" pattern="yyyy-MM-dd HH:mm:ss"/>
                    <div class="wid_5 hei_100 floatleft jkn_message_listtxt"><%=5 * (currentPage - 1) + (i++) + 1%>
                    </div>
                    <div class="wid_10 hei_100 floatleft jkn_message_listtxt"><%="响铃"%>
                    </div>
                    <div class="wid_15 hei_100 floatleft jkn_message_listtxt">${ringUname}</div>
                    ...
                    <div class="wid_15 hei_100 floatleft jkn_message_listtxt">
                        <s:if test="userId != ringUid">
                            <s:if test="userId == 1 || isAdmin ==0">
                                <div class="jkn_message_listdel"
                                     onclick="javascript:delRingbell(${ringId},'${hostids}')"></div>
                            </s:if>
                        </s:if>
                        <s:if test="userId == ringUid">
                        <div class="jkn_message_listedit"
                             onclick="javascript:alterRingbell(${ringId},'${hostids}')"></div>
                        <div class="jkn_message_listdel"
                             onclick="javascript:delRingbell(${ringId},'${hostids}')"></div>
                        </s:if>
                    </div>
...
<script>
    ...
    function alterRingbell(pid, phosts) {
        $("#ringbellid").val(pid);
...
//            hostNameStr += checkedHostsArr.eq(i).prev(".hostoverflow").val() + ",";
        ...
        if (pid == "0") {
            $.post("${pageContext.request.contextPath}/policy/Ringbell_addRingbell.do", {
                /*policyType: ptype,*/
                ringbellLoop: ploop,
                ringbellLooptype: plooptype,
                ringbellDate: pdate,
                ringbellWeek: pweek,
                ringbellTime: ptime,
                ringbellSingletime: psingletime,
                ringbellType:"<%=device_type%>",
                hostStr: hostIdsStr,
                hostIpStr: hostIpStr,
            ...
            $.post("${pageContext.request.contextPath}/policy/Ringbell_alterRingbell.do", {
                ringbellId: pid,
                ringbellLoop: ploop,
                ringbellLooptype: plooptype,
                ringbellDate: pdate,
    ...
    //删除定时计划
    function delRingbell(pid, hosts) {
        $.layer({
            shade: [0.5],
            maxmin: false,
            area: ['310px', '129px'],
            dialog: {
                ...
                btn: ['确定', '取消'],
                yes: function () {
                    $.post("${pageContext.request.contextPath}/policy/Ringbell_delRingbell.do", {
                        ringbellId: pid,
                        hostStr: hosts
                    }, function (data) {
                        layer.msg(data.msg);
