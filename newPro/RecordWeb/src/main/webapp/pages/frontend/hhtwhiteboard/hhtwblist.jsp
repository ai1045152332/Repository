...
%>
<div class="hei_45 wid_50 floatleft showhidden" style="position: relative;margin-top: 1%;">

  <div class="allarea wid_50 floatleft">
    <div class="xk_video"  bingo="none" id="<%=host_id%>" ip="<%=hostip%>">
      <input id="host_cmd_<%=host_id%>" type="hidden" remote_energy="<%=map_cmd.get("remote_energy")%>" touch_screen="<%=map_cmd.get("touch_screen")%>" audio_mode="<%=map_cmd.get("audio_mode")%>" mute_status="<%=map_cmd.get("mute_status")%>" audio_control="<%=map_cmd.get("audio_control")%>" signal="<%=map_cmd.get("signal")%>"
             power="<%=map_cmd.get("power")%>" htpr_standy="<%=map_cmd.get("htpr_standy")%>" htpr_energy="<%=map_cmd.get("htpr_energy")%>">
      <input id="host_cmd_code_<%=host_id%>" type="hidden" remote_energy_cmd="<%=map_cmd.get("remote_energy_cmd")%>" touch_screen_cmd="<%=map_cmd.get("touch_screen_cmd")%>" audio_mode_cmd="<%=map_cmd.get("audio_mode_cmd")%>"
             mute_status_cmd="<%=map_cmd.get("mute_status_cmd")%>"  audio_control_cmd="<%=map_cmd.get("audio_control_cmd")%>" signal_cmd="<%=map_cmd.get("signal_cmd")%>" power_cmd="<%=map_cmd.get("power_cmd")%>" htpr_standy_cmd="<%=map_cmd.get("htpr_standy_cmd")%>" htpr_energy_cmd="<%=map_cmd.get("htpr_energy_cmd")%>">
      <input id="host_status_<%=host_id%>" type="hidden" value="<%=status%>" />
      <input class="hoststatus" type="hidden" value="status_<%=host_id%>_<%=hostip%>" />
      <input id="status_<%=host_id%>" type="hidden" remote_energy="<%=remote_energy%>" touch_screen="<%=touch_screen%>" audio_mode="<%=audio_mode%>" mute_status="<%=mute_status%>" audio_control="<%=audio_control%>" signal="<%=signal%>" top_softWare="<%=top_softWare%>" htpr_standy="<%=htpr_standy%>" htpr_energy="<%=htpr_energy%>"/>
      ...
      <div class="jkn_mc_hover">
        ...
        <div class="jkn_mc_hover_name floatleft">静音状态：<span id="current_mute_status_<%=host_id%>"></span></div>
        <%--<div class="jkn_mc_hover_name floatleft">投影机待机：<span id="current_htpr_standy_<%=host_id%>"></span></div>--%>
        <%--<div class="jkn_mc_hover_name floatleft">投影机省电：<span id="current_htpr_energy_<%=host_id%>"></span></div>--%>
      </div>
      <%}%>
    </div>
  </div>
...
<script>
    ...
    if(hostSize != 0){
      for(var i =0;i<hostSize;i++){
        ...
        if(status == "Online" && hostid != "" && hostip !=""){
          $.post("${pageContext.request.contextPath}/hhtwhiteboard/HHTWhiteboard_getImage.do",
  ...
  $(function(){
    ...
    if(hostSize != 0){
      for(var i =0;i<hostSize;i++){
        ...
        var status = $("#host_status_"+hostid).val();
        if(status == "Online" && hostid != "" && hostip !=""){
          $.post("${pageContext.request.contextPath}/hhtwhiteboard/HHTWhiteboard_getInfo.do",
                  {hostid:hostid,hostIp:hostip},
                  function(data){
                    //console.log(data);
                    for(var j=0;j<data.length;j++) {
                      ...
                      var muteState = data[j].MuteState;
                      ...
                      var htprStandy = data[j].HtprStandy;
                      var htprEnergy = data[j].htprEnergy;
                      if(muteState=="UnMute"){
                        muteState="非静音";
                      ...
                      $("#status_" + hostidstr).attr("mute_status", muteState);
                      $("#status_" + hostidstr).attr("signal", signal);
                      $("#status_" + hostidstr).attr("topSoftWare", topSoftWare);
                      $("#status_" + hostidstr).attr("htpr_standy", htprStandy);
                      $("#status_" + hostidstr).attr("htpr_energy", htprEnergy);
                      ...
                      //投影机待机
                      var currentHtprStandy = "";
                      var htpr_standy = $("#host_cmd_" + hostidstr).attr("htpr_standy");
                      var htpr_standy_cmd = $("#host_cmd_code_" + hostidstr).attr("htpr_standy_cmd");
                      if (htpr_standy != "null") {
                        var arr = htpr_standy.split(",");
                        var arr_cmd = htpr_standy_cmd.split(",");
                        for (var i = 0; i < arr.length; i++) {
                          if (arr_cmd[i] == htprStandy) {
                            currentHtprStandy = arr[i];
                          }
                        }
                        //投影机待机
                        $("#current_htpr_standy_" + hostidstr).html(currentHtprStandy);
                      }
                      //投影机省电

                      var currentHtprEnergy = "";
                      var htpr_energy = $("#host_cmd_" + hostidstr).attr("htpr_energy");
                      var htpr_energy_cmd = $("#host_cmd_code_" + hostidstr).attr("htpr_energy_cmd");
                      if (htpr_energy != "null") {
                        var arr = htpr_energy.split(",");
                        var arr_cmd = htpr_energy_cmd.split(",");
                        for (var i = 0; i < arr.length; i++) {
                          if (arr_cmd[i] == htprEnergy) {
                            currentHtprEnergy = arr[i];
                          }
                        }
                        //投影机省电
                        $("#current_htpr_energy_" + hostidstr).html(currentHtprEnergy);
                      }
                      //音量
                      $("#current_audio_control_" + hostidstr).html(volume);
                      //静音设置
                      $("#current_mute_status_" + hostidstr).html(muteState);
  ...
  $(function () {
    ...
    //非全屏状态下效果
    var iframewidth=$(".hei_45").width();
    var iframeheight=$(".hei_45").height();
    ...
    if(sixtweenwidth>=iframewidth){//高度计算宽度
      ...
      if(sixtweenheight>=iframeheight){//如果高度大于45%
        ...
        for(i=iframewidth;i>0;i--){
          ...
          if(sixtweenheight<=iframeheight){
            ...
            $(".twofour,.xk_video_img,.xk_video_twofour,.xk_video,.xk_video_selected,.jkn_mc_hover").height(sixtweenheight-25)
            $(".xk_video,.xk_video_img,.jkn_mc_hover,.xk_video_selected,.xk_video_twofour,.xk_video_twofour").width(iframewidth)
        ...
        $(".allarea").height(sixtweenheight)
        $(".allarea").width(iframewidth)
        $(".twofour,.xk_video_twofour,.jkn_mc_hover,.xk_video_img,.xk_video,.xk_video_selected").height(sixtweenheight-25)
        $(".xk_video,.jkn_mc_hover,.xk_video_selected,.xk_video_img,.xk_video_twofour,.xk_video_twofour").width(iframewidth)
        var halfwidth=$(".xk_video_twofour").width()/2-1;
        var halfheight=$(".xk_video_twofour").height()/2;
        $(".xk_video_twofourobj").width(halfwidth);
        $(".xk_video_twofourobj").height(halfheight)
      ...
      $(".twofour,.xk_video_twofour,.jkn_mc_hover,.xk_video_img,.xk_video,.xk_video_selected").height(iframeheight-25)
      $(".xk_video,.xk_video_img,.jkn_mc_hover,.xk_video_selected,.xk_video_twofour,.xk_video_twofour").width(sixtweenwidth)
      var halfwidth=$(".xk_video_twofour").width()/2-1;
      var halfheight=$(".xk_video_twofour").height()/2;
      $(".xk_video_twofourobj").width(halfwidth);
      $(".xk_video_twofourobj").height(halfheight)
  ...
  function setval(parames,dapingip,types){
    ...
    for(i=0;i<pinglen;i++){
      if($(".showhidden").eq(i).find(".allarea").children("div").eq(0).attr("ip")==dapingip){//比对ip
        ...
        else if(types =="HtprStandy")
        {
          $("#status_"+cid).attr("htpr_standy",parames);
          var currentHtprStandy = "";
          var htpr_standy = $("#host_cmd_" + cid).attr("htpr_standy");
          var htpr_standy_cmd = $("#host_cmd_code_" + cid).attr("htpr_standy_cmd");
          if (htpr_standy != "null") {
            var arr = htpr_standy.split(",");
            var arr_cmd = htpr_standy_cmd.split(",");
            for (var i = 0; i < arr.length; i++) {
            ...
            //投影机待机
            $("#current_htpr_standy_" + cid).html(currentHtprStandy);
          }
        }
        else if(types =="HtprEnergy")
        {
          $("#status_"+cid).attr("htpr_energy",parames);
          var currentHtprEnergy = "";
          var htpr_energy = $("#host_cmd_" + cid).attr("htpr_energy");
          var htpr_energy_cmd = $("#host_cmd_code_" + cid).attr("htpr_energy_cmd");
          if (htpr_energy != "null") {
            var arr = htpr_energy.split(",");
            var arr_cmd = htpr_energy_cmd.split(",");
            for (var i = 0; i < arr.length; i++) {
            ...
            //投影机省电
            $("#current_htpr_energy_" + cid).html(currentHtprEnergy);
          }
        }
        ...
        else if(types == "MuteStatus"){
          $("#status_"+cid).attr("mute_status", parames);
          $("#current_mute_status_"+cid).html(parames);
        }
