...
<div class="public">
    ...
    <div class="mm_head">
        ...
        <a href="${pageContext.request.contextPath}/news/News_myRichNews.do"  class="mm_head_nummachine">我的</a>
    </div>
    ...
    %>
        ...
        <%
            Page publishNewsList = (Page)request.getAttribute("publishNewsList");
            int pageCount = 0;
            pageCount = publishNewsList.getTotalPageSize();
            ...
            if(publishNewsList != null){
                newsList = publishNewsList.getResult();
                if(newsList != null && newsList.size()>0){
                    for(int i =0;i<newsList.size();i++){
                        try{
                            String nid = newsList.get(i).get("n_id")!=null?newsList.get(i).get("n_id"):"";
                            String uid = newsList.get(i).get("uid");
        ...
        %>
        <div class="sm_list" resid="191" id="news_<%=nid%>">
            <a target="_blank" href="${pageContext.request.contextPath}/msgResource/publish/<%=nid%>/preview/project.html">
            <%--<a href="${pageContext.request.contextPath}/pages/frontend/news/editor/index.jsp?prid=<%=nid%>&w=<%=progra.getW()%>&h=<%=progra.getH()%>">--%>
                <img src="${pageContext.request.contextPath}/msgResource/publish/<%=nid%>/thumb.png" class="sm_listimg">
                <span class="sm_listdate"><%=newsList.get(i).get("n_timeline")==null?"":newsList.get(i).get("n_timeline")%></span>
                <%--<span class="sm_listtext"><span class="sm_listtext30" title="班级名称：初一（223）班">初一（223）班</span><span class="sm_listtext30" title="科目："></span></span>--%>
            ...
            <%
                ...
                if(newsList.get(i).get("uid")!= null && !newsList.get(i).get("uid").equals("") && !newsList.get(i).get("uid").equals(userId)) {//对于非本人发布的信息，无法编辑

            %>
            <div class="mm_list_options mm_list_del" newsId = "<%=nid%>" style="margin-left:20px;margin-top: 10px;" title="删除" hostnumber="0"></div>
            <%
            }else{
            %>
...
//调整卡片间距
    ...
    $(".mm_list_del").click(function(){
        ...
        var url = "${pageContext.request.contextPath}/news/News_richNewsList.do";
