...
function clearPlan()//清空计划 清空本天的还是所有的
{
    layer.confirm("您确定清空所有的录像计划吗?",function() {
        ...
        $(function(){
            ...
            var class_sperate = parseInt(5);
            var five_minites = parseInt(5);
            if(chapterlen>=6){
        ...
        $(".tm_hour").live("click",function(){
                ...
                var thisobj=$(this).parents(".add_wincelveneme_shez_li").find(".tm_class").eq(ind);
                var nextobj=$(this).parents(".add_wincelveneme_shez_li").find(".tm_class").eq(ind+1);
                if(ind>11){
                    $(this).parents(".add_wincelveneme_shez_li").find(".tm_class").eq(ind).find(".choosem").css("top","-227px");
                    $(this).parents(".add_wincelveneme_shez_li").find(".tm_class").eq(ind).find(".chooseh").css("top","-102px")
                }
                //显示小时数
                for(i=0;i<24;i++){
                    if(i<10){
                ...
                //判断当前tm_class的长度
                var classlen=$(this).parents(".add_wincelveneme_shez_li").find(".tm_class").length;
                var starthour=$(this).parents(".add_wincelveneme_shez_li").find(".tm_class").eq(classlen-2).children(".tm_hour").text();
                var endhour=$(this).parents(".add_wincelveneme_shez_li").find(".tm_class").eq(classlen-1).children(".tm_hour").text();
                var prevhour=$(this).parents(".add_wincelveneme_shez_li").find(".tm_class").eq(classlen-3).children(".tm_hour").text();
                var startminites=$(this).parents(".add_wincelveneme_shez_li").find(".tm_class").eq(classlen-2).children(".tm_minites").text();
                var endminites=$(this).parents(".add_wincelveneme_shez_li").find(".tm_class").eq(classlen-1).children(".tm_minites").text();
                var prevminites=$(this).parents(".add_wincelveneme_shez_li").find(".tm_class").eq(classlen-3).children(".tm_minites").text();
                //当点击第一开始时间时，只需要判定第一节课的结束时间
                if(ind==0){
                    var secminites=$(this).parents(".add_wincelveneme_shez_li").find(".tm_class").eq(ind+1).children(".tm_minites").text();
                    var sechour=$(this).parents(".add_wincelveneme_shez_li").find(".tm_class").eq(ind+1).children(".tm_hour").text();
                    var thishour=$(this).parents(".add_wincelveneme_shez_li").find(".tm_class").eq(ind).children(".tm_hour").text();
                    var thisminites=$(this).parents(".add_wincelveneme_shez_li").find(".tm_class").eq(ind).children(".tm_minites").text();
                    //初始化
                    for(m=0;m<24;m++){
                        thisobj.children(".chooseh").children("span").eq(m).css("display","block");
                    }
                    if(sechour!="小时"){
                        for(m=parseInt(sechour);m<24;m++){
                    ...
                    if(starthour=="小时"){
                        $(this).parents(".add_wincelveneme_shez_li").find(".tm_class").eq(ind).children(".chooseh").hide();
                        $(this).parents(".add_wincelveneme_shez_li").find(".tm_class").eq(ind).children(".choosem").hide();
                    ...
                    var starthour=$(this).parents(".add_wincelveneme_shez_li").find(".tm_class").eq(ind).children(".tm_hour").text();
                    if(starthour=="小时"){
                        var starthour=$(this).parents(".add_wincelveneme_shez_li").find(".tm_class").eq(ind).children(".tm_hour").text();
                        var endhour=$(this).parents(".add_wincelveneme_shez_li").find(".tm_class").eq(ind+1).children(".tm_hour").text();
                        var prevhour=$(this).parents(".add_wincelveneme_shez_li").find(".tm_class").eq(ind-1).children(".tm_hour").text();
                        var prevval=parseInt(prevhour);
                        var thisval=parseInt(starthour);
                        var nextval=parseInt(endhour);
                        var startm=parseInt($(this).parents(".add_wincelveneme_shez_li").find(".tm_class").eq(ind).children(".tm_minites").text());
                        var endm=parseInt($(this).parents(".add_wincelveneme_shez_li").find(".tm_class").eq(ind+1).children(".tm_minites").text());
                        var prevm=parseInt($(this).parents(".add_wincelveneme_shez_li").find(".tm_class").eq(ind-1).children(".tm_minites").text());
                        ...
                        //alert("c")
                        var starthour=parseInt($(this).parents(".add_wincelveneme_shez_li").find(".tm_class").eq(ind).children(".tm_hour").text());
                        var endhour=parseInt($(this).parents(".add_wincelveneme_shez_li").find(".tm_class").eq(ind+1).children(".tm_hour").text());
                        var prevhour=parseInt($(this).parents(".add_wincelveneme_shez_li").find(".tm_class").eq(ind-1).children(".tm_hour").text());
                        var startminites=parseInt($(this).parents(".add_wincelveneme_shez_li").find(".tm_class").eq(ind).children(".tm_minites").text());
                        var endminites=parseInt($(this).parents(".add_wincelveneme_shez_li").find(".tm_class").eq(ind+1).children(".tm_minites").text());
                        var prevminites=parseInt($(this).parents(".add_wincelveneme_shez_li").find(".tm_class").eq(ind-1).children(".tm_minites").text());
                    ...
                    if(preval<thisval&&thisval==endval){
                        ...
                        if((thisval+1)==endval&&endminites<=5&&startminites>=55){
                            for(i=0;i>=endval;i++){
                                thisobj.children(".chooseh").children("span").eq(i).css("display","none");
                            }
                        }
            ...
            $(".chooseh span").live("click",function(){
                ...
                var thisobj=$(this).parents(".add_wincelveneme_shez_li").find(".tm_class").eq(ind);
                var nextobj=$(this).parents(".add_wincelveneme_shez_li").find(".tm_class").eq(ind+1);
                var prevobj=$(this).parents(".add_wincelveneme_shez_li").find(".tm_class").eq(ind-1);
                ...
                //当点击小时选项后，需要判断分钟值
                var last=$(this).parents(".add_wincelveneme_shez_li").find(".tm_class").length-1;
                ...
                if(ind==0){
                    if(nexthval!="小时"){
                        if(thishval==nexthval){//相等结束小时
                            if(nextmval!="分钟"){
                                if(parseInt(thismval)>parseInt(nextmval)){
                                    //alert("开始分钟不能大于结束分钟")
                                    $(".tm_warn_setstart").css("display","block");
                                    setTimeout(hidestart,2000);
                                    thisobj.find(".tm_minites").text(parseInt(nextmval)-5)
                                    thisobj.children(".minutetime").val(parseInt(nextmval)-5);
                                }
                            }
                        }else{//小于结束小时;只需处理相邻小时即可
                            if((parseInt(thishval)+1)==parseInt(nexthval)){
                                if(nextmval!="分钟"){
                                    if(parseInt(nextmval)<5&&(parseInt(nextmval)+60-5)<parseInt(thismval)){//当结束时间小于5并且开始时间大于结束时间减去5分钟间隔时
                                        thisobj.find(".tm_minites").text(parseInt(nextmval)+60-5)
                                        thisobj.children(".minutetime").val(parseInt(nextmval)+60-5);
                                    }
                                }
                            }
                        }
                    ...
                    if(parseInt(prevhval)<parseInt(thishval)&&parseInt(thishval)==parseInt(nexthval)){
                        ...
                        if((parseInt(nextmval)-5)<parseInt(thismval)){
                            ...
                            }else if(nextmval >10 && nextmval<60){
                                thisobj.find(".tm_minites").text(parseInt(nextmval))
                                thisobj.children(".minutetime").val(parseInt(nextmval));
                            }else if(nextmval<0){
                                thisobj.find(".tm_hour").text(parseInt(thishval)-1);
                        ...
                        if(parseInt(thismval)<parseInt(prevmval)){
                            ...
                            thisobj.children(".minutetime").val(prevmval);
                            //alert("a")
                        }else if(parseInt(thismval)==parseInt(prevmval)){
                            prevmval = parseInt(prevmval) + five_minites;
                            if(prevmval<10){
                                prevmval = "0"+prevmval;
                            }
                            ...
                            if((parseInt(thismval)-five_minites)<parseInt(prevmval)){
                                prevmval = parseInt(prevmval) + five_minites;
                            if(prevmval<10){
                                prevmval = "0"+prevmval;
                            }
                        ...
                        }else if(parseInt(prevmval)<55&&(parseInt(thishval)-1)==parseInt(prevhval)&&parseInt(thishval)<parseInt(nexthval)){
                            if(parseInt(thishval)==(parseInt(nexthval)-1)){
                               // alert("a"+parseInt(thismval))
                                //需判断分钟值
                                if(parseInt(thismval)<55){

                                    if(parseInt(prevmval)<55){

                                        //alert(startminites)
                                        var thisminites=parseInt(thismval);
                                        thisobj.children(".tm_minites").text(thisminites);
                                        thisobj.children(".minutetime").val(thisminites);
                                    }else{
                                        var thisminites=parseInt(prevmval)+5-60;
                                        if(thisminites<10){
                                            thisminites="0"+thisminites;
                                        }
                                        thisobj.children(".tm_minites").text(thisminites);
                                        thisobj.children(".minutetime").val(thisminites);
                                    }
                                   ...
                                   // alert("b")
                                    if(parseInt(endminites)<5){

                                        var thisminites=parseInt(endminites)+60-5;
                                        thisobj.children(".tm_minites").text(thisminites);
                                        thisobj.children(".minutetime").val(thisminites);
                                    }else{
                                        var thisminites=parseInt(thismval);
                               ...
                               if(parseInt(thismval)<=parseInt(prevmval)){
                                   var thisminites=parseInt(prevmval)+5;
                                }else{
                                   var thisminites=thisminites
                               }
                                thisobj.children(".tm_minites").text(thisminites);
                                thisobj.children(".minutetime").val(thisminites);
                            }
               ...
               // console.log(starthour+"-"+startminites+"-"+endhour+"-"+endminites)
            ...
            $(".tm_minites").live("click",function(){
                ...
                var starthour=$(this).parents(".add_wincelveneme_shez_li").find(".tm_class").eq(classlen-2).children(".tm_hour").text();
                var endhour=$(this).parents(".add_wincelveneme_shez_li").find(".tm_class").eq(classlen-1).children(".tm_hour").text();
                var endhourb=$(this).parents(".add_wincelveneme_shez_li").find(".tm_class").eq(classlen-1).children(".tm_hour").text();
                var prevhour=$(this).parents(".add_wincelveneme_shez_li").find(".tm_class").eq(classlen-3).children(".tm_hour").text();
                var startminites=$(this).parents(".add_wincelveneme_shez_li").find(".tm_class").eq(classlen-2).children(".tm_minites").text();
                var endminites=$(this).parents(".add_wincelveneme_shez_li").find(".tm_class").eq(classlen-1).children(".tm_minites").text();
                var prevminites=$(this).parents(".add_wincelveneme_shez_li").find(".tm_class").eq(classlen-3).children(".tm_minites").text();
                ...
                if(ind==0){
                    ...
                    var endindhour=$(this).parents(".add_wincelveneme_shez_li").find(".tm_class").eq(1).children(".tm_hour").text();
                    var startindhour=$(this).parents(".add_wincelveneme_shez_li").find(".tm_class").eq(0).children(".tm_hour").text();
                    var startindminites=$(this).parents(".add_wincelveneme_shez_li").find(".tm_class").eq(0).children(".tm_minites").text();
                    var endindminites=$(this).parents(".add_wincelveneme_shez_li").find(".tm_class").eq(1).children(".tm_minites").text();
                    ...
                    if(endindminites!="分钟"){
                        ...
                        }else if((parseInt(endindhour)-1)==parseInt(startindhour)){
                                if(parseInt(endindminites)<5){
                                    for(m=0;m<60;m++){
                                        thisobj.children(".choosem").children("span").eq(m).css("display","block");
                                    }
                                    var hideminites=60+parseInt(endindminites)-5
                                    for(m=hideminites;m<60;m++){
                                        thisobj.children(".choosem").children("span").eq(m).css("display","none");
                                    }
                                }else{
                                    for(m=0;m<60;m++){
...
//  
                    ...
                    var endhour=$(this).parents(".add_wincelveneme_shez_li").find(".tm_class").eq(ind+1).children(".tm_hour").text();
                    var prevhour=$(this).parents(".add_wincelveneme_shez_li").find(".tm_class").eq(ind-1).children(".tm_hour").text();
                    var startminites=$(this).parents(".add_wincelveneme_shez_li").find(".tm_class").eq(ind).children(".tm_minites").text();
                    var endminites=$(this).parents(".add_wincelveneme_shez_li").find(".tm_class").eq(ind+1).children(".tm_minites").text();
                    var prevminites=$(this).parents(".add_wincelveneme_shez_li").find(".tm_class").eq(ind-1).children(".tm_minites").text();
            ...
            $(".choosem span").live("click",function(){
                ...
                var tm_classunb = $(this).parents(".add_wincelveneme_shez_li");
...
//                  }
                ...
                if(starthour!="小时"&&endhour!="小时"&&startminites!="分钟"&&endminites!="分钟"){
                   // alert('sss')
                    tm_classunb.find(".tm_addunbind").attr("class","tm_add");
                }
                adduseornouse()
            })

        $(".tm_add").live("click",function(){
                var week = $(this).parents(".add_wincelveneme_shez_main").prev().find("span").attr("value");
                var classlen=$(this).parents(".add_wincelveneme_shez_li").find(".tm_class").length;
                if(classlen<=0){
                    $(".tm_warn_add").show();
                //点击添加按钮添加课时
                //自定义添加课时代码，每次添加一个开始时间和结束时间
                var html="<div class=\"tm_class\" >"+
                                    ...
                                    "<input type='hidden' name='section_"+week+"' value='' class='section_val'/>"+
                                "</div>"+
                ...
                $(this).parents(".add_wincelveneme_shez_li").find(".timeplanlist").append(html);
                }else{
                    $(".tm_warn_add").hide();
                    //点击添加按钮添加课时
                //自定义添加课时代码，每次添加一个开始时间和结束时间
            ...
            $(".week_val").each(function(i){
                $(this).parents(".add_wincelveneme_shez_li").find(".section_val").attr("name","section_"+(i+1));
            })
                //添加完课时后隐藏添加按钮，或者添加一个替换的div,禁用点击事件
                $(this).parents(".add_wincelveneme_shez_li").find(".tm_add").attr("class","tm_addunbind");
                var classlen=$(this).parents(".add_wincelveneme_shez_li").find(".tm_class").length;
                //给课时添加课时数
                if(classlen%2==0){
...
//                      }
                    ...
                    var section = count;
                    $(this).parents(".add_wincelveneme_shez_li").find(".tm_class").eq(classlen-2).children(".chapter").text("第"+zhangjie+"节");

                    $(this).parents(".add_wincelveneme_shez_li").find(".tm_class").eq(classlen-2).children(".chapter").next().attr("value",count);
                    //$(this).parents(".add_wincelveneme_shez_li").find(".tm_class").eq(classlen-2).children(".chapter").next().attr("value",count);
                }
                for(i=0;i<classlen;i++){
                    if(i==(classlen-1)){
                        $(this).parents(".add_wincelveneme_shez_li").find(".tm_class").eq(i).children(".chapterdel").css("display","block");
                    }else{
                        $(this).parents(".add_wincelveneme_shez_li").find(".tm_class").eq(i).children(".chapterdel").css("display","none");
                    }
                }
                var starthour=$(this).parents(".add_wincelveneme_shez_li").find(".tm_class").eq(classlen-2).children(".tm_hour").text();
                var endhour=$(this).parents(".add_wincelveneme_shez_li").find(".tm_class").eq(classlen-1).children(".tm_hour").text();
            ...
            //超过20课时，不允许添加
            var chapterlen=$(this).parents(".add_wincelveneme_shez_li").find(".chapter").length;
            if(chapterlen>=20){
                $(this).parents(".add_wincelveneme_shez_li").find(".tm_addunbind").hide();
            }else{
                $(this).parents(".add_wincelveneme_shez_li").find(".tm_addunbind").show();
...
$(".chapterdel").live("click",function(){
     $(this).parents(".add_wincelveneme_shez_li").find(".tm_addunbind").attr("class","tm_add");
    //不超过20课时，允许添加
    var chapterlen=$(this).parents(".add_wincelveneme_shez_li").find(".chapter").length;
    //alert(chapterlen)
    if(chapterlen>=20){
    ...
    $(this).parents(".add_wincelveneme_shez_li").find(".tm_addunbind").show();
    $(this).parents(".add_wincelveneme_shez_li").find(".tm_addunbind").attr("class","tm_add");
    $(this).parents(".add_wincelveneme_shez_li").find(".tm_class").eq(prevind-1).children(".chapterdel").show();
    $(this).parents(".add_wincelveneme_shez_li").find(".tm_class").eq(prevind).remove();
    $(this).parents(".add_wincelveneme_shez_li").find(".tm_class").eq(prevind).remove();
...
function adduseornouse(){
    var classlen=$(".tm_class").length;
    if(parseInt($(".tm_class").eq(classlen-1).find(".tm_hour").text())==23 && parseInt($(".tm_class").eq(classlen-1).find(".tm_minites").text())>49){
        $(".tm_add").addClass("tm_addunbind").removeClass("tm_add")
    }
}