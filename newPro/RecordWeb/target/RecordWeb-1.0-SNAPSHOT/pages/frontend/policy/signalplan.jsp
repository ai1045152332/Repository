...
//    Map<String, Object> listmap = (Map<String, Object>) request.getAttribute("policyList");
//    String pageCount = listmap.get("pageCount").toString();
//    int currentPage = Integer.parseInt(request.getAttribute("currentPage").toString());
//    List<Object[]> policylist = (List<Object[]>) listmap.get("policylist");
...
<div class="public">
            ...
            <div class='contentr'>
                <div class="tree">
                    ...
                    <div class="public_left">
                        <s:iterator value="#groupTree">
                            <s:if test="hostInfoList!=null&& hostInfoList.size!=0">
                            <div class="tree_title tree_title_close ">
                                ...
                                <div class="tree_content" style="display: none">
                                    ...
                                    <s:iterator value="%{hostInfoList}" id="inner">
                                        <div class="tree_contenta"  hostId="${id}" hostIp="${token}" hostName="${name}">
                                            <s:if test="%{status=='Offline'}">
                                                <span class="tree_content_nousebg" style="background: none;"></span>
                                            </s:if>
                                            <s:else>
                                                <span class="tree_content_onlinebg" style="background: none;"></span>
                                            </s:else>
                                            <span class="hostoverflow">${name}</span>

                                            <div class="jkn_treecheckbox" id="jkn_checkbox_${id}" hostId="${token}" hostIp="${host_ip}" hostName="${name}"
                                                 status="${status}" dspec="${dspecid}" hostIp="${host_ip}"
                                                   onclick="changecheckBox()" ></div>
                                        </div>
                                    </s:iterator>
    ...
    <div class="public_right floatleft">
        ...
        <div class="jkn_message_rightcenter">
            <input type="hidden" id="ringbellid" value="0"/>
            ...
            <div class="jkn_message_text" style="margin-top: -3px;margin-left: 30px;width:auto">信号切换：</div>
                <div class="jkn_message_radio2" style="margin-top: 0; width:auto">
                    <div class="headr">
                        ...
                        <div id="signal_select" class="sall" style="margin-top:-8px;margin-left:-5px;">
                            ...
                            <div class="selectdivall" id="selectdivall1">
                                ...
                                <div class="selectdivul" id="selectdivul1" style="z-index: 11;"></div>
                            </div>
                        </div>
                    </div>
                </div>
        ...
        <div class="jkn_message_rightcenter hei_50 floatleft" style="margin-left: 5%;">
            ...
            <%
                int i = 0;
            %>
            <s:iterator value="#signalplanList" status='st'>
                <div id="${signalId}"
                     signal="${signal}"
                     signalCode="${signalCode}"
                     isloop="${signalLoop}"
                     looptype="${signalLooptype}"

                     date="${signalDate}"
                     week="${signalWeek}"
                     time="${signalTime}"
                     singletime="${signalSingletime}"
                     uid="${signalUid}"

                     class="<%=i%2==0?"jkn_message_listwhite floatleft":"jkn_message_listgray floatleft"%>">
                    <fmt:parseDate value="${signalCreatetime}" var="parsedEmpDate" pattern="yyyy-MM-dd HH:mm:ss"/>
                    <div class="wid_5 hei_100 floatleft jkn_message_listtxt"><%=5 * (currentPage - 1) + (i++) + 1%>
                    </div>
                    <div class="wid_10 hei_100 floatleft jkn_message_listtxt"><%="信号切换"%>
                    </div>
                    <div class="wid_15 hei_100 floatleft jkn_message_listtxt">${signalUname}</div>
                    <div class="wid_30 hei_100 floatleft jkn_message_listtxt">${hosts} </div>
                    <div class="wid_20 hei_100 floatleft jkn_message_listtxt"><fmt:formatDate value="${parsedEmpDate}" pattern="yyyy-MM-dd HH:mm:ss"/></div>
                    <div class="wid_15 hei_100 floatleft jkn_message_listtxt">
                        <s:if test="userId != signalUid">
                            <s:if test="userId == 1 || isAdmin ==0">
                                <div class="jkn_message_listdel"
                                     onclick="javascript:delSignalplan(${signalId},'${hostids}')"></div>
                            </s:if>
                        </s:if>
                        <s:if test="userId == signalUid">
                        <div class="jkn_message_listedit"
                             onclick="javascript:alterSignalplan(${signalId},'${hostids}')"></div>
                        <div class="jkn_message_listdel"
                             onclick="javascript:delSignalplan(${signalId},'${hostids}')"></div>
                        </s:if>
                    </div>
        ...
        <input type="hidden" id="temp_cmd" value=""/>
...
<script>
    ...
    function alterSignalplan(pid, phosts) {
        $("#ringbellid").val(pid);
        /*  var ptype = $("#" + pid).attr("type");*/
        var psingal = $("#" + pid).attr("signal");
        var psingalcode = $("#" + pid).attr("signalCode");
        ...
        if (phosts != "") {
            ...
            selectauto(psingalcode);
        }
    }
    //点击发布按钮
    $(".jkn_message_sendbtn").click(function () {
        var pid = $("#ringbellid").val();
...
//           hostNameStr += checkedHostsArr.eq(i).prev(".hostoverflow").val() + ",";
        ...
        if ($("#select1").find("option:selected").text() == "选择信号源") {
            layer.msg("请选择信号源");
            return false;
        }
        ...
        psingnalCode = $("#select1").val();
        if (pid == "0") {
            $.post("${pageContext.request.contextPath}/policy/Signalplan_addSignalplan.do", {
                /*policyType: ptype,*/
                signal: psingnal,
                signalCode: psingnalCode,
                signalLoop: ploop,
                signalLooptype: plooptype,
                signalDate: pdate,
                signalWeek: pweek,
                signalTime: ptime,
                hostIpStr: hostIpStr,
                signalSingletime: psingletime,
                hostStr: hostIdsStr,
            ...
            $.post("${pageContext.request.contextPath}/policy/Signalplan_alterSignalplan.do", {
                signalId: pid,
                signal: psingnal,
                signalCode: psingnalCode,
                signalLoop: ploop,
    ...
    //删除定时计划
    function delSignalplan(pid, hosts) {
        $.layer({
            shade: [0.5],
            maxmin: false,
            area: ['310px', '129px'],
            dialog: {
                ...
                btn: ['确定', '取消'],
                yes: function () {
                    $.post("${pageContext.request.contextPath}/policy/Signalplan_delSignalplan.do", {
                        signalId: pid,
                        hostStr: hosts
                    }, function (data) {
                        layer.msg(data.msg);
    ...
    $("#signal_select").click(
            function () {
                ...
                if (hostidstr == 0) {
                    ...
                    $("#selectdiv1").empty();
                    $("#selectdivul1").empty();
                    $("#select1").html("<option  value='0' >选择信号源</option>");
                    changeselect(1);
                    return false;
                }
                //获取选中大屏的vga名称
...
//                    }
                ...
                if ($("#temp_cmd").val() != arr_cmd) {

                    $("#temp_cmd").val(arr_cmd);
                    var str = "<option value='0' >选择信号源</option>";
                    //if(status == "Online"){
                    for (var i = 0; i < arr_cmd.length; i++) {
                        if (arr_cmd[i] != current) {
                            ...
                            if (arr_cmd[i]) {
                                str += "<option value=" + arr_cmd[i] + " >" + arr[i] + "</option>";
                            ...
                            if (current != "") {
                                str += "<option value=" + arr_cmd[i] + " selected=‘selected’ >" + arr[i] + "</option>";
    ...
    function selectauto(siganlcode) {
        var hostidstr = $(".jkn_treecheckboxed").length;
