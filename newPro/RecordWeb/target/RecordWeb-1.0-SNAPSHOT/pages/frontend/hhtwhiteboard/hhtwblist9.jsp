...
<div class="hei_30 wid_33 floatleft showhidden" style="position: relative;margin-top: 1%;">
  <div class="allarea wid_33 floatleft">
    <div class="xk_video"  bingo="none" id="<%=host_id%>" ip="<%=hostip%>">
      <input id="host_cmd_<%=host_id%>" type="hidden" remote_energy="<%=map_cmd.get("remote_energy")%>" touch_screen="<%=map_cmd.get("touch_screen")%>" audio_mode="<%=map_cmd.get("audio_mode")%>" mute_status="<%=map_cmd.get("mute_status")%>" audio_control="<%=map_cmd.get("audio_control")%>" signal="<%=map_cmd.get("signal")%>" power="<%=map_cmd.get("power")%>" htpr_standy="<%=map_cmd.get("htpr_standy")%>" htpr_energy="<%=map_cmd.get("htpr_energy")%>">
      <input id="host_cmd_code_<%=host_id%>" type="hidden" remote_energy_cmd="<%=map_cmd.get("remote_energy_cmd")%>" touch_screen_cmd="<%=map_cmd.get("touch_screen_cmd")%>" audio_mode_cmd="<%=map_cmd.get("audio_mode_cmd")%>" mute_status_cmd="<%=map_cmd.get("mute_status_cmd")%>" audio_control_cmd="<%=map_cmd.get("audio_control_cmd")%>" signal_cmd="<%=map_cmd.get("signal_cmd")%>" power_cmd="<%=map_cmd.get("power_cmd")%>" htpr_standy_cmd="<%=map_cmd.get("htpr_standy_cmd")%>" htpr_energy_cmd="<%=map_cmd.get("htpr_energy_cmd")%>">
      ...
      <div class="jkn_mc_hover">
        ...
        <%--<% if ("8".equals(host.get("dspec"))){//当设备为901.6设备时 支持触控切换 所以显示此项 %>--%>
