    ...
    <div class="allarea wid_50 floatleft">
        <div class="xk_video"  bingo="none" id="<%=host_id%>" ip="<%=hostip%>">
            <input id="host_cmd_<%=host_id%>" type="hidden" remote_energy="<%=map_cmd.get("remote_energy")%>"
                   touch_screen="<%=map_cmd.get("touch_screen")%>"
                   audio_mode="<%=map_cmd.get("audio_mode")%>"
                   audio_control="<%=map_cmd.get("audio_control")%>"
                   signal="<%=map_cmd.get("signal")%>"
                   power="<%=map_cmd.get("power")%>">
            <input id="host_cmd_code_<%=host_id%>" type="hidden" remote_energy_cmd="<%=map_cmd.get("remote_energy_cmd")%>"
            ...
            <div class="jkn_mc_hover">
                ...
                <div  class="jkn_mc_hover_name floatleft">投影机状态：<span id="current_turnState_<%=host_id%>"></span></div>
                ...
                <div class="jkn_mc_hover_name floatleft" style="display: none">置顶软件：<span id="current_top_softWare_<%=host_id%>"></span></div>
            </div>
            <%}%>
        </div>
    </div>
                    ...
                    $.post("${pageContext.request.contextPath}/htprojector/HTProjector_getInfo.do",
                            ...
                            function(data){
                                ...
                                for(var j=0;j<data.length;j++) {
                                    ...
                                    $("#current_top_softWare_" + hostidstr).attr("title", topSoftWare);
                                    var currentSignal = "";
