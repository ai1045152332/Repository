...
<div class="public">
    ...
    <div class="public_left22 floatleft">
        ...
        <div id="otp_vedeoabout_rvideo">
            <div class='contentr'>
                <div class="tree">
                    <a href="javascript:void(0)" class="tree_titleb ">所有设备</a>
                    ...
                    <div class="public_left">
                        ...
                        <div class="tree_title tree_title_close">
                            <span class="tree_titlea" groupId="<%=group_id%>"
                                  style="text-indent: 48px;"><%=group_name%></span>
    ...
    <div class="public_right floatleft">
        ...
        <div class="jkn_message_rightcenter">
            ...
            <div class="jkn_message_90 floatleft">
                ...
                <div class="jkn_message_radio2" style="margin-top: 0; margin-left: 30px;width:auto">
                    <div class="headr">
                        <span id="boot" class="bgr"></span>开机
                    </div>
                    <div class="headr">
                        <span id="shutdown" class="bgr"></span>关机
                    </div>
        ...
        <div class="jkn_message_rightcenter hei_50 floatleft" style="margin-left: 5%;">
            ...
            <div id="<%=policy[0]%>" type="<%=policy[1]%>" isloop="<%=policy[2]%>"
                ...
                <div class="wid_10 hei_100 floatleft jkn_message_listtxt"><%=Integer.parseInt(policy[1].toString()) == 0 ? "关机" : "开机"%>
                </div>
                <%
                    String userName = "";
                    int newuser = -1;
                ...
                <div class="wid_15 hei_100 floatleft jkn_message_listtxt">
                    ...
                    %>
                    <div class="jkn_message_listdel"   onclick="javascript:delPolicy(<%=policy[0]%>,'<%=policy[10]%>')"></div>
                    <%
                            }
                        }else
                            {
                    %>
                    <div class="jkn_message_listedit"  onclick="javascript:alterPolicy(<%=policy[0]%>,'<%=policy[10]%>')"></div>
                    <div class="jkn_message_listdel"   onclick="javascript:delPolicy(<%=policy[0]%>,'<%=policy[10]%>')"></div>
                <%
                         }
                %>
...
//    $(document).on("click",".jkn_message_listedit",function(){
//        $(".jkn_message_listedit").css("display","block")
...
//    })
    ...
    function alterPolicy(pid, phosts) {
        var currentElement;
//        if (currentElement) {
...
//        }
       // console.log(obj)
        ...
        $("#policyid").val(pid);
//        $("div").addClass("jkn_message_listedited").removeClass("jkn_message_listedit");

//        $("div").siblings().addClass("jkn_message_listedit").removeClass("jkn_message_listedited")
...
//            hostNameStr+= checkedHostsArr.eq(i).prev(".hostoverflow").val() + ",";
        ...
        if(pid == "0")
        {
            $.post("${pageContext.request.contextPath}/policy/Policy_addPolicy.do",{
                policyType:ptype,
                policyLoop:ploop,
                policyLooptype:plooptype,
                policyDate:pdate,
                policyWeek:pweek,
                policyTime:ptime,
                policySingletime:psingletime,
                policyDevicetype:"<%=device_type%>",
                hostStr:hostIdsStr,
                hostIpStr: hostIpStr,
        ...
        {
            $.post("${pageContext.request.contextPath}/policy/Policy_alterPolicy.do",{
                policyId:pid,
                policyType:ptype,
                policyLoop:ploop,
                policyLooptype:plooptype,
    ...
    function delPolicy(pid,hosts)
    {
        $.layer({
            ...
            dialog: {
                ...
                btn: ['确定','取消'],
                yes: function(){
                    $.post("${pageContext.request.contextPath}/policy/Policy_delPolicy.do",{policyId:pid,hostStr:hosts},function(data){
                        layer.msg(data.msg);
                        setTimeout(function(){
                            if (data.success == true) {
                                window.location.reload();
