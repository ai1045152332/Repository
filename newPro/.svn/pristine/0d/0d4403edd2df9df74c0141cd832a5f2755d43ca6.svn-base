...
<style type="text/css">
  ...
  .ztree li a.curSelectedNode {background-color:#D4D4D4;border:0;height:30px;width: 100%;}
  .ztree li span {line-height:30px;}
  .ztree li span.button {margin-top: -7px;}
  .ztree li span.button.switch {width: 16px;height: 16px;}
  .ztree li ul.level0 {padding:0; background:none;width: 100%;}
  input[type="checkbox"]{
  	display: block;
  }
  .ztree li ul li a{
...
//    debugger;
    ...
    var allVirtual1="${virtualPreView1}";
    var currentPathId="${currentPathId}";
    var virtualArray =allVirtual.split("/");
    var virtualArray1 =allVirtual1.split("/");
    var newVirtual = "";
    var urlItem = "";
    for (var i = 1; i < virtualArray.length - 1; i++) {
      urlItem += "" == urlItem ? virtualArray[i] : "/" + virtualArray1[i];
      var param="/"+urlItem+"/";
      var linkVirtual = "<a href='${pageContext.request.contextPath}/tresource/Tresource_home.do?module=Resource&virtualPreView="
              + param + "'>" + (virtualArray[i]=="root"?"公共资源":virtualArray1[i]) + "</a>";
      newVirtual += "" == newVirtual ? linkVirtual : "  \\  " + linkVirtual;
    }
    $('#disTitle').html("<span class=\'master-header-span\' id=\'playplanContentHeader\' style=\'margin-top:0px\' '>"+newVirtual+"</span>");
...
//    }
  ...
  function downloadItems() {
    ...
    if (num == 0) {
      ...
      for ( var i = 0; i < rows.length; i++) {
        if(rows[i].type == 0) {
          $.messager.alert('提示', '请不要选择文件夹进行下载操作!', 'info');
          return;
        }
      }
      /*        var n = 0;
      ...
      for (var i = 0; i < num; i++) {
        ...
        //window.location.href=encodeURI(requestContextPath+"/msgResource/data/resources/"+pathFolder+"/"+name+'?n='+contentName);
        var suffix = name.lastIndexOf(".") >= 0 ? name.substring(name.lastIndexOf(".")) : "";
        var myUrl = requestContextPath+"/tresource/Tresource_downloadResource.do?name=" +(contentName + suffix) + "&url=" + encodeURI(encodeURI(requestContextPath+"/msgResource/data/resources/0/" + name));
        //alert(myUrl);
        //tresource/Tresource_downloadResource.do?name=Maven%E5%92%8CNexus%E4%BD%BF%E7%94%A8%E5%9F%B9%E8%AE%AD%E6%96%87%E6%A1%A3_%E5%BC%A0%E7%BA%A2%E5%BD%AC_20150722.ppt&url=%2FmsgResource%2Fdata%2Fresources%2F1%2Fa522640b57ec875d9b285974dce0dbf0.ppt
        window.open(myUrl, "_blank", "");
//        setTimeout(function(){
//         var name = arrayObj[n];
//         var suffix = name.lastIndexOf(".") >= 0 ? name.substring(name.lastIndexOf(".")) : "";
//         var myUrl = requestContextPath+"/tresource/Tresource_downloadResource.do?name=" + encodeURIComponent(contentName + suffix) + "&url=" + encodeURIComponent(requestContextPath+"/data/resources/0/" + name);
//         window.open(myUrl, "_blank", "");
//         n++;
//         }, n * 100);
      }
...
//      $("#"+md5na3+"bar").html(Math.floor(bytesUploaded*100/bytesTotal)+'%');
$("#"+md5na3+"bar").attr("class","loading")
      ...
      'onSelect' : function(file) {
        ...
        $("#uploadFlag").val("1");
        /*  $.ajax({
         url:'${pageContext.request.contextPath }/curriculum/ResourceUploadByUploadify_verifyOneNameOnly.do',
         type:'post',
         data:{
         nameStr:str,
         pathid:pathid
        ...
        if(arrayO.length>0){
          ...
          if(exist<0){ //数组不包含元素
            arrayO.push(obj);
            $("#aryo").val(arrayO);
            $("#p3").append("<div class='2' id='"+md5na3+"' style='position:relative; border:1px #CCC solid;  height:48px; width:690px; margin:0 auto; padding:1px'> <div id='"+md5na+"' style=' background:#3F9; height:48px; width:0px'></div><strong  style='position:absolute; width:690px; top:7px; overflow:hidden'><div title="+na+" style='height:48px;width:340px;float:left;font-weight:normal'>"+nasu+"</div> <div style='height:48px;width:105px;float:left;font-weight:normal'>"+Math.floor(file.size/1024)+"kb</div><div style='height:48px;width:115px;float:left;font-weight:normal'></div> <div id='"+md5na3+"bar"+"'style='height:48px;width:50px;float:left;font-weight:normal'></div> <div><a name="+md5na3+" id="+md5na3+"pro"+" class='resourceUpload-cancel'/></div></strong> </div>");
          }
          if(exist>=0){ //数组包含元素
        ...
        /* }else{
         $.messager.alert('错误',file.name+'文件名称已经存在，请更改', 'error',function(){
         sign = 1;
         $('#file_upload').uploadify('cancel', file.id,'0');
         });
      ...
      'onSelectError': function (file, errorCode, errorMsg) {
        switch (errorCode) {
          case -100:
            ...
            break;
          case -110:
            $.messager.alert('提示',"文件大小超出系统限制的" + "1Gb"+ "大小！", 'info');
            break;
          case -120:
            $.messager.alert('提示',"文件大小异常！", 'info');
            break;
          case -130:
...
//文件列表上传完成时间        
      'onQueueComplete' : function(queueData) {
        ...
        this.settings.queueData.uploadsSuccessful = 0;
        this.settings.queueData.uploadsErrored = 0;
        cancelPathSelect2();
        $("#uploadFlag").val("0");
        $("#aryo").val("");
      },
  ...
  function bb(flag) {
    ...
    if(flag=="pubdels"){
      ...
      $('#disTitle').html("<span class=\'master-header-span\' id=\'playplanContentHeader\'  >"+"公共资源"+"</span>");
    }
    ...
    var viruecode=encodeURI(encodeURI(viru));
    $('#resourceList').show();
    dg.datagrid(
            {
              border : false,
              ...
              columns : [ [
                {
                 ...
                 // hidden : true,
                  formatter : function(value, row, index) {
                    ...
                    switch (row.type) {
                      case 0:
                        var vir=encodeURIComponent(encodeURIComponent(row.virtual+row.name+'/'));
                  ...
                  title : '文件大小',
                  formatter:function(value, row, index){
                    var filesize = row.size==0?"":(row.size/1024).toFixed(2)+"M";
                    if(filesize == "0.00M")
                    {
                      filesize = "0.01M";
                    }
  ...
  function bbupload() {
    ...
    var mytime=myDate.toLocaleTimeString();
    dg= $('#dg');
    var virucode=encodeURI(encodeURI('${virtualPreView}'));
    //var viruecode=encodeURI(encodeURI(viru));
    $('#resourceList').show();
    dg.datagrid(
            {
              ...
              pageList : [ 10, 20, 30, 40 ],
              iconCls : 'icon-save',
              url : '${pageContext.request.contextPath}/tresource/Tresource_load1.do?virtual='+virucode,
              onLoadSuccess : function(d){
                var size = dg.datagrid('getRows').length;
                if(!searchFlag){//搜索
                  if(size==0){
              ...
              columns : [ [
                {
                  ...
                  formatter : function(value, row, index) {
                    if(row.transStatus==3){
                      ...
                      return "<font color='#7B7B7B'>转码中 </font><img src='${pageContext.request.contextPath}/image/frontend/resources/resource-uploading.gif'/>";
                    }{//转换失败
...
//    }
  ...
  function valid() {
    $('#ff')
            .form(
            {
              url : '${pageContext.request.contextPath}/tresource/TresourceUploadByUploadify_md5Validate.do',//update by caoqian
              onSubmit : function() {
...
//    }
    ...
    if (num == 0) {
      ...
      $.messager.alert('提示','您最多只能同时选择一条记录进行修改!', 'info');
      $('#dg').datagrid('clearSelections');
      return;
    } else {
  ...
  function changeDivMy(){
    ...
    $("#divFlag").val("my");
    $('#disTitle').html("<span class=\'master-header-span\' id=\'playplanContentHeader\'  >我上传的</span>");
  ...
  function changeDivVerify(){
    $('#disTitle').html("<span class=\'master-header-span\' id=\'playplanContentHeader\'  >待审核资源</span>");
  ...
  function previewOpen(id,type,name){
    ...
    var pathForQuery="/msgResource/data/resources/"+pathFolder+"/"+foldername+"/";
    content = '<span class="edit-resources-span">'+name+'</span><br/>';
    switch (type) {
...
//                  "	<source src='"+requestContextPath+"/msgResource/data/resources/"+pathFolder+"/"+foldername+"/video.mp4'>"+
//                  "	<object data='movie.mp4' width='500' height='500'> "+
//                  "     <embed src='"+requestContextPath+"/msgResource/data/resources/"+pathFolder+"/"+foldername+"/video.mp4'>"+
//                  "	  </object>"+
//                  "</video>";
//        }else{
          content+=
                  "<object data='"+requestContextPath+"/msgResource/data/resources/"+pathFolder+"/"+foldername+"/video.mp4' height='500' width='500'/>";
        //}
        break;
      case 2:
        $.ajax({
        ...
        for(var i=1;i<=num;i++){
          content+="<img class=\"edit-resources-image\" src=\""+requestContextPath+"/msgResource/data/resources/"+pathFolder+"/"+foldername+"/"+i+".png\"></img>";
        }
        $("#previewEditNum").val("");
        break;
      case 3:
      ...
      case 4:
        if(nameType.toLowerCase()=="jpg"||nameType.toLowerCase()=="bmp"){
          content+="<img class=\"edit-resources-image\" src=\""+requestContextPath+"/msgResource/data/resources/"+pathFolder+"/"+foldername+"/image.png\"></img><br>";
        }else{
          content+="<img class=\"edit-resources-image\" src=\""+requestContextPath+"/msgResource/data/resources/"+pathFolder+"/"+foldername+"/image."+nameType+"\"></img><br>";
        }
      ...
      case 7:
        var fla=
                ...
                "	<param name=\"movie\" value=\""+requestContextPath+"/msgResource/data/resources/"+pathFolder+"/"+foldername+"/flash."+nameType+"\" />"+
                ...
                "	<embed width=\"480px\" height=\"360px\" src=\""+requestContextPath+"/msgResource/data/resources/"+pathFolder+"/"+foldername+"/flash."+nameType+"\" quality=\"high\" play=\"true\" loop=\"true\" wmode=\"transparent\" scale=\"showall\" menu=\"false\" type=\"application/x-shockwave-flash\"/> "+
                " </object><br>" ;
        content+=fla;
            ...
            function(data) {
              if (data) {
                ...
                var suffix = name.lastIndexOf(".") >= 0 ? name.substring(name.lastIndexOf(".")) : "";
                 ...
                 var myUrl = requestContextPath+"/tresource/Tresource_downloadResource.do?name=" + encodeURI(encodeURI(previewName + suffix)) + "&url=" + encodeURI(encodeURI(requestContextPath+"/msgResource/data/resources/0/" + name));
                 window.open(myUrl, "_blank", "");
              }
            }
    );
...
<div class="master-content">
  ...
  <div id="disTitle" i="pheader" class="master-contentheader" style="height: 40px;font-size: 16px;line-height: 40px;"></div>
