...
function clearPlan()//清空计划 清空本天的还是所有的
{
    layer.confirm("您确定清空所有的录像计划吗?",function() {
		...
		$(".tm_hour").live("click",function(){
				...
				if(ind==0){
                    var secminites=$(".tm_class").eq(ind+1).children(".tm_minites").text();
                    var sechour=$(".tm_class").eq(ind+1).children(".tm_hour").text();
                    var thishour=$(".tm_class").eq(ind).children(".tm_hour").text();
                    var thisminites=$(".tm_class").eq(ind).children(".tm_minites").text();
                    //初始化
                    for(m=0;m<24;m++){
					...
					if(starthour=="小时"){
						...
						//alert("c")
						var starthour=parseInt($(".tm_class").eq(ind).children(".tm_hour").text());
						var endhour=parseInt($(".tm_class").eq(ind+1).children(".tm_hour").text());
						var prevhour=parseInt($(".tm_class").eq(ind-1).children(".tm_hour").text());
...
//					}
		...
		$(".tm_add").live("click",function(){
				...
				var classlen=$(".tm_class").length;
				$("#vipt_video1 .scrollson").height($("#vipt_video1 .timeplanlist").height()+260)
				scroll_y("vipt_video1","scrollson","scroll_y","scroll_ymove","scroll_x","scroll_xmove","","wheely","")	
...
$(".chapterdel").live("click",function(){
	var classlen=$(".tm_class").length;
	$("#vipt_video1 .scrollson").height($("#vipt_video1 .timeplanlist").height()+260)
	scroll_y("vipt_video1","scrollson","scroll_y","scroll_ymove","scroll_x","scroll_xmove","del","wheely","")	
	if(classlen<=4){
		$("#vipt_video1 .scrollson").css("margin-top","0")
		$("#vipt_video1 .scroll_y").css("top","0")
	}
    //不超过20课时，允许添加
    var chapterlen=$(".chapter").length;
    if(chapterlen>=20){
...
$(".vip_cframe_input").live("blur",function(){
    ...
    $(".vipt_time").live("mouseover",function(){
        $(this).css({"height":"46px","width":"115px","box-shadow":"0 0 0 2px #28b779 inset"})
    }).live("mouseout",function(){
        $(this).css({"height":"46px","width":"115px","box-shadow":"0 0 0 2px #fff inset"})
    })
    ...
    $(".vipt_timegreen").live("mouseover",function(){
       ...
       $(this).css({"height":"46px","width":"115px","box-shadow":"0 0 0 2px #F1F8F2 inset"})
    })
